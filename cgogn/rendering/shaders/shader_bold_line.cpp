...
ShaderBoldLineColorNoTB* ShaderBoldLineColorNoTB::instance_ = nullptr;

ShaderBoldLineColorNoTB::ShaderBoldLineColorNoTB()
{
	const char* vertex_shader_source = R"(
		#version 330
		in vec3 vertex_position;
	...
	const char* fragment_shader_source = R"(
		...
		void main()
		{
			...
			float lambert = max(1.0 - lighted, Nz); // Nz = dot(N,0,0,1)
			frag_out = col * lambert;
		}
	)";

	load3_bind(vertex_shader_source, fragment_shader_source, geometry_shader_source, "vertex_position",
			   "vertex_color", "clipping_position");
	get_uniforms( "line_width", "lighted", "plane_clip", "plane_clip2");
}

void ShaderParamBoldLineColorNoTB::set_uniforms()
{
	int viewport[4];
	glGetIntegerv(GL_VIEWPORT, viewport);
	GLVec2 width(width_ / float32(viewport[2]), width_ / float32(viewport[3]));
	shader_->set_uniforms_values(width, lighted_, plane_clip_, plane_clip2_);
}
