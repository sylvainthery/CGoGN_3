...
void ShaderParamExplodeVolumesGenerateShadows::bind_texture_buffers()
{
...
void ShaderParamExplodeVolumesGenerateShadows::release_texture_buffers()
{
...
ShaderExplodeVolumesShadows::ShaderExplodeVolumesShadows()
{
	...
	const char* fragment_shader_source = R"(
		...
		void main()
		{
			...
			float k = gl_FragCoord.z;//*0.5+0.5;
			frag_out = vec4(lambert * color.rgb, color.a)*0.000001+vec4(mix(vec3(1,0,0),vec3(0,1,0),k),1.0);
		}
	)";
