...
{
	foreach_incident_edge(m, c, func, MapBase_TraversalPolicy::AUTO);
}

template <typename MESH, typename CELL, typename FUNC>
auto foreach_incident_edge(const MESH& m, CELL c, const FUNC& func, MapBase_TraversalPolicy traversal_policy)
	-> std::enable_if_t<std::is_convertible_v<MESH&, struct MapBase&>>
{
	...
	{
		if (traversal_policy == MapBase_TraversalPolicy::AUTO && is_indexed<Edge>(m))
		{
