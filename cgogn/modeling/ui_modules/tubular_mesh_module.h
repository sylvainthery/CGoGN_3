	...
	void extend_graph_extremities()
	{
		...
		foreach_cell(cache, [&](GraphVertex v) -> bool {
			if (degree(*graph_, v) == 1)
			{
				...
				if (h.hit)
				{
					typename mesh_traits<GRAPH>::MeshType& gra = *graph_;
					GraphVertex nv = add_vertex(gra);
					
					connect_vertices(gra, v, nv);
					value<Vec3>(gra, graph_vertex_position_, nv) = p + 0.6 * (h.pos - p);
				}
			}
			return true;
		});
